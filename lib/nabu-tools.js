...
Goblin.registerQuest(goblinName, 'translate', function* (quest, next) {
  ...
  for (const app of apps) {
    ...
    for (const row of rows) {
      const {id, source, fr_CH, de_CH} = row;
      ...
      if (fr_CH) {
        yield quest.cmd('nabu.set-translatable-data-translation', {
          ...
          nabuId: source,
          localeName: 'fr_CH',
          translation: removeContext(fr_CH),
        });
        quest.log.dbg(`fr_CH: ${removeContext(fr_CH)}`);
      }
      ...
      if (de_CH) {
        yield quest.cmd('nabu.set-translatable-data-translation', {
          ...
          nabuId: source,
          localeName: 'de_CH',
          translation: removeContext(de_CH),
        });
        quest.log.dbg(`de_CH: ${removeContext(de_CH)}`);
      }
