...
const retrieveEntity = watt(function* (r, table, documentId) {
  try {
    return yield r.get({
      table,
      documentId,
...
const Tr = watt(function* (quest, localeName, msg, fromCache) {
  ...
  const message = storageRethink
    ? translated ||
      (yield retrieveEntity(storageRethink, 'nabuMessage', messageId))
    : translated;
