...
class StringBuilder {
  static joinWords(...args) {
    if (args.length === 1 && Array.isArray(args[0])) {
      args = args[0];
    }
    return StringBuilder._join(args, ' ');
  }

  static joinSentences(...args) {
    if (args.length === 1 && Array.isArray(args[0])) {
      args = args[0];
    }
    return StringBuilder._join(args, ', ');
  ...
  static joinHyphen(...args) {
    if (args.length === 1 && Array.isArray(args[0])) {
      args = args[0];
    }
    return StringBuilder._join(args, ' — '); // U+2014: tiret cadratin
  }

  static joinLines(...args) {
    if (args.length === 1 && Array.isArray(args[0])) {
      args = args[0];
    }
    return StringBuilder._join(args, '\n\n');
