...
const extractMessages = function* (
  ...
  */
  ...
  const persistedMessages = (yield r.getAll({
    ...
    .filter((message) =>
      message.sources.some((source) => source.appName === appName)
    )
    .map((message) => {
  ...
  if ($outputFile) {
    ...
    let dataForTranslator = Papa.unparse(dataToSerialize);
    dataForTranslator = '\ufeff' + dataForTranslator;
    fs.writeFileSync($outputFile, dataForTranslator);

    const jsonOutput = $outputFile.replace('.csv', '.json');
    fs.writeFileSync(jsonOutput, JSON.stringify(dataToSerialize, ' ', 2));
