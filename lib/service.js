...
const logicHandlers = {
  ...
  'set-selected-item': (state, action) => {
    return state.set(`selectionMode.selectedNabuId`, action.get('nabuId'));
  },
...
Goblin.registerQuest(goblinName, 'open-datagrid', function*(quest, next) {
  ...
  const datagridId = yield desk.addWorkitem({workitem, navigate: true}, next);
  quest.goblin.setX('datagridId', datagridId);
});

Goblin.registerQuest(
  goblinName,
  'add-message',
  (quest, workitemId, nabuId, description) => {
    ...
    e.index({
      ...
      quest.createFor(messageId, workitemId, messageId, {
        id: messageId,
        nabuId,
...
for (const action of Object.keys(logicHandlers)) {
  switch (action) {
    ...
    case 'set-selected-item':
      Goblin.registerQuest(goblinName, action, function*(quest, nabuId, next) {
        const enabled = quest.goblin.getState().get('enabled');
        ...
        if (enabled) {
          if (datagridId) {
            try {
              ...
              yield datagridApi.resetListVisualization(
                {
                  filter: {nabuId},
                },
                next
              ...
              if (err.code !== 'XCRAFT_CMD_ERROR') {
                // otherwise, it is just because the datagrid has been closed
                throw err;
              }
            }
          }
