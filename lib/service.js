...
Goblin.registerQuest(goblinName, 'get-messages', function(quest) {
  var state = quest.goblin.getState();
  var messages = state.get('messages');

  return messages;
});

Goblin.registerQuest(goblinName, 'init', function*(quest, appName, disabled) {
  if (!appName) {
