...
const deleteMessage = watt(function*(quest, message, desktopId, next) {
...
const patchMessage = watt(function*(quest, message, desktopId, next) {
  ...
  yield entity.apply({patch: message}, next);
});
...
Goblin.registerQuest(goblinName, 'extract-messages', function*(
  ...
  */
  ...
  const messagesToDelete = _.without.apply(
    _,
    [currentMessagesIds].concat(messagesIds)
  );
  const messagesToAdd = _.without.apply(
    _,
    [messagesIds].concat(currentMessagesIds)
  );
  ...
  const messagesMap = {};
  _(currentMessages.concat(messages))
    .groupBy(msg => msg.id)
    .map(group => {
      return {
        id: group[0].id,
  ...
  for (let msgId of messagesToDelete) {
    deleteMessage(quest, messagesMap[msgId], desktopId, next.parallel());
  }
