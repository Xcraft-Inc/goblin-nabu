...
Goblin.registerQuest(goblinName, 'handle-elastic-indexes', function*(
  ...
  if (configuration && storageAvailable) {
    const locales = (yield quest.me.getLocales()).toArray();

    const multilanguageIndexes = [configuration.mandate].concat(
      locales.map(
        locale =>
          `${configuration.mandate}-${locale
            .get('name')
            .toLowerCase()
            .replace(/\//g, '-')}`
      )
    ...
    for (let index of multilanguageIndexes) {
...
Goblin.registerQuest(goblinName, 'handle-locales-change', function*(
  ...
  yield quest.me.handleElasticIndexes({
    configuration: {
      mandate: elasticConfig.mandate,
      url: elasticConfig.url,
      reset: false,
      types: elasticConfig.types,
    },
  });
});
...
Goblin.registerQuest(goblinName, 'init-desktop', function*(
  ...
  yield quest.me.handleElasticIndexes({configuration: elasticConfig});

  yield quest.me.unpackMessages({desktopId, elasticConfig});
  ...
  if (!noStorage) {
    ...
    yield r.startQuestOnChanges({
      ...
      goblinId: quest.goblin.id,
      questArgs: {desktopId, elasticConfig},
...
Goblin.registerQuest(goblinName, 'deinit-desktop', function*(quest, desktopId) {
  const storageAvailable = quest.goblin.getX('storageAvailable');
  if (storageAvailable) {
