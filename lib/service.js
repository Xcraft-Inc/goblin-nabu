...
const {crypto, modules} = require('xcraft-core-utils');
...
const {
  ...
  patchMessage,
  deleteMessage,
} = require('./messages-helpers.js');

function retrieveJsFiles(appId) {
  const appDir = path.join(xHost.projectPath, 'app');
  const libDir = path.join(xHost.projectPath, 'node_modules');
  const configJson = modules.loadAppConfig(appId, appDir);
  const deps = modules.extractAllDeps(appId, libDir, configJson);
...
const extractFileMessages = watt(function*(
  ...
  const relativePath = path.relative(xHost.projectPath, filePath);

  const opts = {
    presets: ['es2015', 'react', 'stage-0'],
    plugins: [
      ['./lib/goblin-nabu/lib/extractor.js', {appName, file: relativePath}],
    ],
  };
...
Goblin.registerQuest(goblinName, 'extract-messages', function*(
  ...
  */
  ...
  if (!process.env.WESTEROS_APP) {
    throw new Error(
      'Nabu extract-messages: env variable WESTEROS_APP is undefined'
    );
  }

  if (!process.env.WESTEROS_APP.includes(appName)) {
    throw new Error(
      `Nabu extract-messages: missmatch between WESTEROS_APP (${
        process.env.WESTEROS_APP
      }) and appName (${appName})`
    );
  }

  const paths = retrieveJsFiles(process.env.WESTEROS_APP);
