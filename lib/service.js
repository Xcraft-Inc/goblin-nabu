...
Goblin.registerQuest(goblinName, 'try-add-locales', function* (
  ...
  locales = quest.goblin.getState().get('locales').toJS().concat(localesToAdd);
  ...
  for (let locale of localesToAdd) {
    ...
    yield quest.createFor(locale.id, 'nabu', locale.id, {
      id: locale.id,
      status: 'published',
      name: locale.name,
      text: locale.text,
...
Goblin.registerQuest(goblinName, 'add-message', function* (
  ...
  const entityApi = yield quest.createFor(messageId, workitemId, messageId, {
  ...
  if (messageId) {
    ...
    quest.goblin
      ...
      .forEach((locale) => {
        ...
        promises.push(
          quest.createFor(translationId, ownerId || messageId, translationId, {
            id: translationId,
            messageId,
...
Goblin.registerQuest(goblinName, 'add-translatable-data', function* (
  ...
  yield quest.createFor(messageId, entityId || desktopId, messageId, {
    desktopId,
    id: messageId,
    nabuId,
    custom: true,
  ...
  quest.goblin
    ...
    .forEach((locale) => {
      ...
      promises.push(
        quest.createFor(translationId, messageId, translationId, {
          id: translationId,
          messageId,
          locale: locale.get('name'),
...
Goblin.registerQuest(goblinName, 'getMandate', function (quest) {
  return 'nabu';
});
