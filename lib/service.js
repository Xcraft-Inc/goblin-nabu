...
Goblin.registerQuest(goblinName, 'init', function*(
  ...
  quest.goblin.setX('nabuDesktopId', `nabu@${mandate}@${quest.uuidV4()}`);
...
const doAndIndexIfEnabled = (quest, messageId, description) => {
  const enabled = quest.goblin.getState().get('enabled');
  if (enabled) {
    quest.do();
    /*const e = quest.getAPI('elastic@nabu');
    ...
    quest.create(msgId, {
      ...
      desktopId: quest.goblin.getX('nabuDesktopId'), // logically it should be desktopId, but it doesn't work...
    });
  }
};
