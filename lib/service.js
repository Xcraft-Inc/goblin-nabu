...
Goblin.registerQuest(goblinName, 'remove-all-translations', function* (
  quest,
  desktopId,
  ownerId,
  messageId,
...
Goblin.registerQuest(goblinName, 'remove-all-locales', function* (
  quest,
  desktopId,
  next
) {
  const storageAvailable = quest.goblin.getX('storageAvailable');
  if (!storageAvailable) {
    throw new Error('Cannot remove locales because storage is not available');
  }
