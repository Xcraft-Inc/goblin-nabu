...
Goblin.registerQuest(goblinName, 'init-desktop', function*(
  ...
  if (!mandate && storageAvailable) {
    mandate = configuration.mandate;
    if (!mandate) {
      throw new Error('Nabu init-error: no mandate provided');
    }
  }
  ...
  yield quest.me.handleElasticIndexes({mandate, reset, configuration});
  ...
  if (storageAvailable) {
    ...
    yield r.startQuestOnChanges({
      ...
      goblinId: quest.goblin.id,
      questArgs: {desktopId, mandate, configuration},
