...
const {pPath, appId} = require('xcraft-core-host');
...
Goblin.registerQuest(goblinName, 'init', function*(
  ...
  quest.defer(() => quest.me.unpackMessages());
...
Goblin.registerQuest(goblinName, 'pack-messages', function*(
  ...
  if (Object.keys(messages).length > 0) {
    ...
    yield fs.mkdir(resourcesPath, {recursive: true}, next);
    yield fs.writeFile(
...
Goblin.registerQuest(goblinName, 'unpack-messages', function*(quest, next) {
  const translationsPath =
    process.env.NODE_ENV === 'production'
      ? path.resolve(projectPath, cacheFileName)
      : path.resolve(projectPath, 'app', appId, 'resources', cacheFileName);

  try {
    const translations = yield fs.readFile(translationsPath, next);

    quest.me.setTranslations(
      {translations: Goblin.Shredder.fromJS(JSON.parse(translations))},
      next
    );
  } catch (err) {
    if (err.code === 'ENOENT') {
