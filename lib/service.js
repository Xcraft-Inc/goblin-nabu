...
function getUniqueSourceString(source) {
  return `${source.appName}.${source.path}.{${source.location.start.line},${
    source.location.start.column
  }}`;
}

function updateSources(oldSources, newSources) {
  return _(oldSources.concat(newSources))
    .uniqBy(source => getUniqueSourceString(source))
    .value();
}

function updateMarkedSources(sources) {
  ...
  return updateSources(oldSources, newSources);
}
...
Goblin.registerQuest(goblinName, 'add-message', function*(
  ...
  yield entity.change(
        {
          path: 'sources',
          newValue: updateSources(persistedSources, currentSources),
        },
        next
      );*/
});
