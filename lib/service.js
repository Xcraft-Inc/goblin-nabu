...
const trySetLocales = watt(function*(quest, locales, desktopId, next) {
  ...
  if (storageAvailable) {
    for (let locale of locales) {
      if (!currLocales.find(currLocale => currLocale.get('id') === locale.id)) {
    ...
    if (shouldReload) {
      yield loadLocales(quest, desktopId);
    }
  } else {
...
const loadLocales = watt(function*(quest, desktopId) {
  const session = desktopId ? desktopId.split('@')[1] : null;
  const r = quest.getStorage('rethink', session);
...
Goblin.registerQuest(goblinName, 'handle-changes', function*(quest, desktopId) {
  yield loadLocales(quest, desktopId);
});
