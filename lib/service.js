...
Goblin.registerQuest(goblinName, 'add-message', function*(
  ...
  const desktopId = extractDesktopId(workitemId);
  if (!desktopId) {
    console.warn(`cannot extract desktopId from workitem ${workitemId}`);
    return;
  }
  ...
  quest.defer(() => quest.me.loadTranslations({messageId, desktopId}));
});
/*
