...
const packMessages = function* (quest, desktopId, $appName, $appId, next) {
  ...
  if ($appId) {
    outputPath = path.resolve(projectPath, 'app', $appId, 'resources');
  }

  if (Object.keys(messages).length > 0) {
    yield fs.ensureDir(outputPath, next);
    yield fs.writeFile(
      outputPath,
      stringify(
        {
